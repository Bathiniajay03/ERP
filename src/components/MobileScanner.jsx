...
//     currentSeq += qty;
//     localStorage.setItem('erp_serial_seq', currentSeq.toString());
...
//   const handlePoReceivingWithSerials = async () => {
//     if (!currentPoLine || !txForm.quantity || !serialGenerationForm.generatedSerials.length) {
//       return showStatus('error', 'Ensure PO line, quantity, and serials are provided.');
//     }
...
//       const response = await api.post(`/purchase-orders/${selectedPo.id}/receive`, payload);
//       showStatus('success', response.data?.message || 'PO received successfully.');
...
//       loadPendingPurchaseOrders();
//     } catch (err) {
//       showStatus('error', err.response?.data?.message || 'PO receiving failed.');
//     } finally {
//       setLoading(false);
//     }
//   };
