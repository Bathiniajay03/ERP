...
// Mocked @zxing/library for standalone environment
...
// Mocked API client for standalone environment
const api = {
  get: async (url, options) => {
    if (url.includes('/purchase-orders/pending')) return { data: [] };
    if (url.includes('/stock/lots')) return { data: [] };
    if (url.includes('/items/barcode/')) {
  ...
  post: async (url, data) => {
    return { data: { message: "Mock transaction successful" } };
  }
};

const SCAN_COOLDOWN_MS = 2000;
let sessionBatchCounter = Math.floor(Math.random() * 26); // Start at a random letter to avoid repeats on refresh
...
export default function MobileScanner({
  ...
  const generateSerialNumbers = () => {
    ...
    // Use A as the default prefix if none is found
    const itemCode = currentItem?.itemCode || 'A';
    const serialPrefix = (currentItem?.serialPrefix || itemCode).trim().toUpperCase().replace(/[^A-Z0-9]/g, '') || 'A';
    ...
    // Cycle through A-Z for each new transaction batch to uniquely identify them
    const batchAlpha = String.fromCharCode(65 + (sessionBatchCounter % 26));
    sessionBatchCounter++;
    
    let serials;
    
    // Different format for PO vs manual stock based on your exact requirement
    if (activeTab === 'po') {
      // PO Format: PO260329A1, PO260329A2...
      serials = Array.from({ length: qty }, (_, i) => {
        return {
          serialNumber: `PO${yy}${mm}${dd}${batchAlpha}${i + 1}`,
          status: 'Available'
        };
      });
    } else {
      // Manual IN/OUT/TRANSFER Format: A260329A1, A260329A2...
      serials = Array.from({ length: qty }, (_, i) => {
        return {
          serialNumber: `${serialPrefix}${yy}${mm}${dd}${batchAlpha}${i + 1}`,
          status: 'Available'
        };
      });
    }
    ...
    setSerialGenerationForm(prev => ({ ...prev, generatedSerials: serials }));
    showStatus('success', `✓ Generated ${qty} serial number(s) sequence`);
  };
