...
export default function MobileScanner({
  ...
  // --- Serial Selection States (For Dispatch/Out) ---
  ...
  useEffect(() => { loadPendingPurchaseOrders(); }, [loadPendingPurchaseOrders]);
  ...
  useEffect(() => { loadAvailableLots(); }, [loadAvailableLots]);

  // --- Fetch Serials for Lot (Auto-select Logic) ---
  const fetchSerialsForLot = useCallback(async () => {
    if (!txForm.lotId || !txForm.itemId) return;
    setSerialLoading(true);
    try {
      const res = await api.get('/stock/serials', { 
        params: { itemId: txForm.itemId, lotId: txForm.lotId, warehouseId: txForm.warehouseId } 
      });
      const serials = res.data || [];
      setAvailableSerials(serials);
      
      // AUTO-SELECT LOGIC
      const qty = parseInt(txForm.quantity) || 0;
      if (qty > 0) {
        const toSelect = serials
          .filter(s => s.status === 'AVAILABLE')
          .slice(0, qty)
          .map(s => s.id);
        setSelectedSerialIds(new Set(toSelect));
      }
    } catch (err) {
      showStatus('error', 'Failed to load serials');
    } finally {
      setSerialLoading(false);
    }
  }, [txForm.lotId, txForm.itemId, txForm.quantity, txForm.warehouseId, showStatus]);

  useEffect(() => {
    if ((txMode === 'out' || txMode === 'transfer') && txForm.lotId) {
      fetchSerialsForLot();
    }
  }, [txForm.lotId, txMode, fetchSerialsForLot]);
  ...
  const autoSelectSerials = () => {
    ...
    const toSelect = available.slice(0, qty).map(s => s.id);
    setSelectedSerialIds(new Set(toSelect));
    showStatus('success', `Auto-selected ${toSelect.length} serials`);
  };
  ...
  const resolveBarcode = useCallback(async (barcodeValue) => {
    ...
    try {
      ...
      setTxForm(prev => ({
        ...prev,
        itemId: String(payload.itemId),
        warehouseId: String(payload.inventory?.warehouseId ?? defaultWarehouseId),
        quantity: trimmed === lastScannedBarcode ? String(parseInt(prev.quantity) + 1) : "1"
      }));
      ...
      if (err?.response?.status === 404) {
        stopCamera();
        setProductForm({...createEmptyProductForm(), barcode: trimmed});
        setActiveTab('create');
      }
    }
  }, [defaultWarehouseId, lastScannedBarcode, showStatus, stopCamera]);
  ...
  const startCamera = useCallback(async () => {
    if (!videoRef.current) return;
    try {
      codeReaderRef.current = codeReaderRef.current || new BrowserMultiFormatReader();
      const deviceId = await findBackCamera();
      codeReaderRef.current.decodeFromVideoDevice(deviceId, videoRef.current, (result) => {
        if (result) handleDetectedCode(result.getText());
      });
      setCameraActive(true);
    } catch (err) { setCameraError('Camera unavailable'); }
  }, [handleDetectedCode, findBackCamera]);

  useEffect(() => {
    if (activeTab !== 'create') startCamera();
    else stopCamera();
    return stopCamera;
  }, [activeTab, startCamera, stopCamera]);
  ...
  const openSerialGenerationModal = () => {
    ...
    if (!txForm.itemId || qty <= 0) return showStatus('error', 'Item and Quantity required.');
    setSerialGenerationForm({ quantity: qty, generatedSerials: [] });
    setShowSerialModal(true);
  };

  const generateSerialNumbers = () => {
    const qty = serialGenerationForm.quantity;
    const prefix = (currentItem?.serialPrefix || 'SN').toUpperCase();
    const dateStr = new Date().toISOString().slice(2,10).replace(/-/g,'');
    const serials = Array.from({ length: qty }, (_, i) => ({
      serialNumber: `${prefix}-${dateStr}-${Math.floor(Math.random() * 10000).toString().padStart(4, '0')}`,
      status: 'Available'
    }));
    setSerialGenerationForm(prev => ({ ...prev, generatedSerials: serials }));
  };
  ...
  const handleStockTransaction = async (e) => {
    ...
    try {
      ...
      // Handle Serials for Receipt
      if (serialGenerationForm.generatedSerials.length > 0) {
        payload.serialNumbers = serialGenerationForm.generatedSerials.map(s => s.serialNumber);
      }

      // Handle Serials for Dispatch
      if (selectedSerialIds.size > 0) {
        payload.serialNumbers = availableSerials
          .filter(s => selectedSerialIds.has(s.id))
      ...
      if (activeTab === 'po') {
        ...
        payload = {
          purchaseOrderLineId: currentPoLine.lineId,
          quantity: payload.quantity,
          lotNumber: txForm.lotNumber || `LOT-${Date.now().toString().slice(-6)}`,
          serialNumbers: payload.serialNumbers || null,
          generateSerials: !!payload.serialNumbers
        };
      } else {
        endpoint = `/stock/${txMode}`;
        if (txMode !== 'in') payload.lotId = parseInt(txForm.lotId);
        if (txMode === 'in') payload.lotNumber = txForm.lotNumber;
        if (txMode === 'transfer') payload.destinationWarehouseId = parseInt(txForm.destWarehouseId);
      }

      const response = await api.post(endpoint, payload);
      showStatus('success', 'Transaction Successful');
      resetScanState();
      setShowSerialModal(false);
      fetchData();
    } catch (err) {
  ...
  return (
    <div className="bg-white p-3 h-100 d-flex flex-column erp-app-wrapper">
      {/* TABS */}
      <ul className="nav nav-tabs mb-3 flex-nowrap overflow-auto">
        <li className="nav-item">
          <button className={`nav-link fw-bold ${activeTab === 'stock' ? 'active text-primary' : ''}`} onClick={() => setActiveTab('stock')}>📦 Stock</button>
        </li>
        <li className="nav-item">
          <button className={`nav-link fw-bold ${activeTab === 'po' ? 'active text-primary' : ''}`} onClick={() => setActiveTab('po')}>🛒 PO Recv</button>
        </li>
      </ul>

      {/* SCANNER */}
      {activeTab !== 'create' && (
        <div className="position-relative bg-dark rounded shadow-sm mb-3" style={{ height: '200px' }}>
          <video ref={videoRef} className="w-100 h-100 object-fit-cover" autoPlay muted playsInline />
          <div className="position-absolute top-50 start-50 translate-middle w-75 border-top border-success scanner-laser"></div>
        </div>
      )}

      {status.text && <div className={`alert py-2 small fw-bold mb-3 ${status.type === 'success' ? 'alert-success' : 'alert-danger'}`}>{status.text}</div>}

      {/* FORM AREA */}
      {activeTab === 'stock' && (
        <div className="flex-grow-1 overflow-auto">
          <div className="btn-group w-100 mb-3">
            <button className={`btn btn-sm ${txMode === 'in' ? 'btn-success' : 'btn-outline-success'}`} onClick={() => setTxMode('in')}>IN</button>
            <button className={`btn btn-sm ${txMode === 'out' ? 'btn-danger' : 'btn-outline-danger'}`} onClick={() => setTxMode('out')}>OUT</button>
          </div>

          <div className="mb-3">
            <label className="erp-label">Quantity</label>
            <input type="number" className="form-control erp-input" value={txForm.quantity} onChange={(e) => setTxForm({...txForm, quantity: e.target.value})} />
          </div>

          {txMode !== 'in' && (
            <div className="mb-3">
              <label className="erp-label">Source Lot</label>
              <select className="form-select erp-input" value={txForm.lotId} onChange={(e) => setTxForm({...txForm, lotId: e.target.value})}>
                <option value="">-- Select Lot --</option>
                {availableLots.map(l => <option key={l.lotId} value={l.lotId}>{l.lotNumber} ({l.quantity})</option>)}
              </select>
            </div>
          )}

          {/* DISPATCH SERIAL AUTO-SELECT UI */}
          {(txMode === 'out' || txMode === 'transfer') && txForm.lotId && (
            <div className="p-3 border rounded bg-light mb-3">
              <div className="d-flex justify-content-between align-items-center mb-2">
                <span className="fw-bold small">Serial Selection ({selectedSerialIds.size}/{txForm.quantity})</span>
                <button className="btn btn-xs btn-primary py-0" onClick={autoSelectSerials}>Auto-Select</button>
              </div>
              <div style={{ maxHeight: '150px', overflowY: 'auto' }}>
                {availableSerials.map(s => (
                  <div key={s.id} className="form-check small">
                    <input className="form-check-input" type="checkbox" checked={selectedSerialIds.has(s.id)} onChange={() => toggleSerialSelection(s.id)} />
                    <label className="form-check-label">{s.serialNumber}</label>
                  </div>
                ))}
              </div>
            </div>
          ...
          <button className="btn btn-primary w-100 fw-bold" onClick={txMode === 'in' ? openSerialGenerationModal : handleStockTransaction} disabled={loading || !txForm.itemId}>
            {loading ? 'Processing...' : 'Execute Transaction'}
          </button>
        </div>
      )}

      {/* PO TAB (Simplified for focus on fix) */}
      {activeTab === 'po' && (
        <div className="flex-grow-1">
          <select className="form-select mb-3" value={selectedPoNumber} onChange={(e) => setSelectedPoNumber(e.target.value)}>
            <option value="">-- Select PO --</option>
            {poList.map(po => <option key={po.poNumber} value={po.poNumber}>{po.poNumber}</option>)}
          </select>
          <input type="number" className="form-control mb-3" placeholder="Qty" value={txForm.quantity} onChange={(e) => setTxForm({...txForm, quantity: e.target.value})} />
          <button className="btn btn-success w-100 mb-2" onClick={openSerialGenerationModal}>1. Assign Serials</button>
          <button className="btn btn-primary w-100" onClick={handleStockTransaction}>2. Confirm Receipt</button>
        </div>
      )}

      {/* SERIAL MODAL (For Receipt/In) */}
      {showSerialModal && (
        <div className="erp-modal-overlay">
          <div className="erp-dialog p-3">
            <h6 className="fw-bold border-bottom pb-2">Generate Serials ({serialGenerationForm.quantity})</h6>
            <button className="btn btn-sm btn-outline-primary mb-3" onClick={generateSerialNumbers}>Generate Sequence</button>
            <div className="overflow-auto mb-3" style={{maxHeight:'200px'}}>
               {serialGenerationForm.generatedSerials.map((s,i) => <div key={i} className="small font-monospace">{s.serialNumber}</div>)}
            </div>
            <div className="d-flex gap-2">
              <button className="btn btn-light border flex-grow-1" onClick={() => setShowSerialModal(false)}>Cancel</button>
              <button className="btn btn-primary flex-grow-1" onClick={handleStockTransaction} disabled={!serialGenerationForm.generatedSerials.length}>Commit</button>
            </div>
          </div>
        </div>
      )}
      ...
      <style>{`
        .erp-modal-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 2000; display: flex; align-items: center; justify-content: center; }
        .erp-dialog { background: white; width: 90%; border-radius: 8px; }
        .scanner-laser { animation: scan 2s infinite linear; }
        @keyframes scan { 0% { top: 20%; } 50% { top: 80%; } 100% { top: 20%; } }
        .btn-xs { font-size: 0.7rem; padding: 2px 5px; }
        .erp-label { font-size: 0.75rem; font-weight: bold; color: #666; text-transform: uppercase; }
      `}</style>
    </div>
  );
}